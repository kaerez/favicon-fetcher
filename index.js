...
function findRedisUrl() {
  // 1. Prioritize the standard REDIS_URL if set explicitly.
  if (process.env.REDIS_URL) {
    return process.env.REDIS_URL;
  }
  // 2. Look for a Heroku-provided Redis URL.
  for (const key in process.env) {
    if (key.startsWith('HEROKU_REDIS_')) {
      return process.env[key];
    }
  }
  // 3. Return null if no Redis URL is found.
  return null;
}
...
// Read all other env vars into a case-insensitive map
const env = {};
...
// Use original process.env for case-sensitive keys
...
function parseMultiLimit(limitStr) {
  if (!limitStr) return [];
  const separator = String(limit_separator);
  return String(limitStr).toLowerCase().split(separator).map(part => {
